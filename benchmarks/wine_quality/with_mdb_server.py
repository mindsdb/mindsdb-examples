...
high_percentile_confidence = np.percentile(prediction_confidence_arr, 85)

# The real values
test_data = pd.read_csv(open('processed_data/test.csv', 'r'))
real_val_arr = list(test_data['price'])
real_val_arr = [x if str(x) != 'nan' or x <= 0 else 0.001 for x in real_val_arr]
exact_prediction_arr_no_nan  = [x if str(x) != 'nan' and x is not None and x <= 0 else 0.001 for x in real_val_arr]
...
accuracy = r2_score(np.log(real_val_arr), np.log(exact_prediction_arr_no_nan))
print(f'Got an r2 score of the logs of the price: {accuracy}')
...
for i, cr in enumerate(predicted_range_arr):
    ...
    if confidence > high_percentile_confidence and real_val_arr[i] < cr[0]:
