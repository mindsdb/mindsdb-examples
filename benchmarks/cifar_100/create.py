...
def create_csv(name):
    with open(f'{name}.csv', 'w', newline='\n') as csv_fp:
        csv_writer = csv.writer(csv_fp, delimiter=',')
        csv_writer.writerow(['image_path','superclass','class'])
        for superclass in os.listdir(name):
            for _class in os.listdir(os.path.join(name, superclass)):
                for file in os.listdir(os.path.join(name, superclass,_class)):
                    csv_writer.writerow([os.path.join(name, superclass, _class, file), superclass, _class])
